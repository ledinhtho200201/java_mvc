...
<head>
    ...
    <style>
        ...
        .header-img { width: 44px; height: 44px; border-radius: 8px; object-fit: cover; border: 2px solid rgba(255,255,255,0.3); flex-shrink: 0; }
        .header-img-ph { width: 44px; height: 44px; border-radius: 8px; background: var(--accent); display: flex; align-items: center; justify-content: center; color: #fff; font-size: 1.1rem; flex-shrink: 0; }
        ...
        .current-img-wrap { display: flex; align-items: center; gap: 1rem; margin-bottom: 0.8rem; }
        .current-img { width: 60px; height: 60px; border-radius: 8px; object-fit: cover; border: 3px solid #e5e7eb; }
        .current-img-ph { width: 60px; height: 60px; border-radius: 8px; background: #f4f6fb; display: flex; align-items: center; justify-content: center; color: #bbb; font-size: 1.4rem; }
        ...
        .upload-area i.upload-icon { font-size: 1.8rem; color: #ccc; }
        .upload-area.has-file i.upload-icon { color: var(--accent); }
        .upload-area small { color: #9ca3af; font-size: 0.78rem; }
        #imagePreview { width: 70px; height: 70px; object-fit: cover; border-radius: 8px; display: none; margin: 0.5rem auto 0; }
...
<div class="main-wrap">
    <div class="topbar">
        <div class="topbar-title">
            <a href="/admin/product" style="color:#8a92a6;text-decoration:none;font-size:.85rem;font-weight:400;">Sản phẩm</a>
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        <c:choose>
                            <c:when test="${not empty product.image}">
                                <img src="/images/product/${product.image}" alt="${product.name}" class="header-img"/>
                            </c:when>
                            <c:otherwise>
                                <div class="header-img-ph"><i class="bi bi-box-seam"></i></div>
                            </c:otherwise>
                        </c:choose>
                        <div>
                            <h5><i class="bi bi-pencil-square me-2"></i>Cập nhật sản phẩm</h5>
                            <div style="color:rgba(255,255,255,0.5);font-size:.8rem;">${product.name}</div>
                        </div>
                    </div>
                    <div class="form-card-body">
                        ...
                        <form:form method="POST" action="/admin/product/update"
                            ...
                            <div class="row g-3">
                                <div class="col-12">
                                    <label class="form-label-s">Tên sản phẩm <span class="required">*</span></label>
                                    <form:input type="text" path="name" cssClass="form-control" placeholder="Tên sản phẩm"/>
                                </div>

                                <div class="col-md-6">
                                    <label class="form-label-s">Giá (VNĐ) <span class="required">*</span></label>
                                    <form:input type="number" path="price" cssClass="form-control" placeholder="35990000"/>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label-s">Số lượng <span class="required">*</span></label>
                                    <form:input type="number" path="quantity" cssClass="form-control" placeholder="50"/>
                                </div>

                                <div class="col-md-6">
                                    <label class="form-label-s">Nhà sản xuất <span class="required">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label-s">Mục đích sử dụng <span class="required">*</span></label>
                                ...
                                <div class="col-12">
                                    <label class="form-label-s">Mô tả ngắn</label>
                                    <form:input type="text" path="shortDesc" cssClass="form-control" placeholder="Mô tả ngắn"/>
                                </div>

                                <div class="col-12">
                                    <label class="form-label-s">Mô tả chi tiết</label>
                                    <form:textarea path="detailDesc" cssClass="form-control" rows="4" placeholder="Mô tả chi tiết sản phẩm"/>
                                </div>

                                <div class="col-12">
                                    <label class="form-label-s">Ảnh sản phẩm</label>
                                    <div class="current-img-wrap">
                                        <c:choose>
                                            <c:when test="${not empty product.image}">
                                                <img src="/images/product/${product.image}" alt="current" class="current-img"/>
                                            </c:when>
                                            <c:otherwise>
                                                <div class="current-img-ph"><i class="bi bi-image"></i></div>
                                            </c:otherwise>
                                        </c:choose>
                                    ...
                                    <div class="upload-area" id="uploadArea" onclick="document.getElementById('imageInput').click()">
                                        ...
                                        <div id="uploadText" style="font-size:.85rem;color:#6b7080;margin-top:.3rem;">Nhấn để chọn ảnh mới</div>
                            ...
                            <div class="d-flex gap-2 mt-4">
                                ...
                                <a href="/admin/product" class="btn btn-light px-4">Hủy</a>
                            </div>
                        </form:form>
                    </div>
                </div>
